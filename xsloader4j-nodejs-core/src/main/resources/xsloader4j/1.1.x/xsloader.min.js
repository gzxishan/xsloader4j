/*!
 * xsloader.min.js v1.1.12
 * home:https://github.com/gzxishan/xsloader#readme
 * (c) 2018-2020 gzxishan
 * Released under the Apache-2.0 License.
 * build time:Thu Feb 13 2020 18:06:46 GMT+0800 (GMT+08:00)
 */
(function () {
  'use strict';
  ...
  var L$h=U.global.xsloader;L$h.define("name",{isSingle:true,pluginMain:function pluginMain(a,b,c){var d=a.indexOf("=>>");if(-1==d){c("expected:=>>");return}var e=a.substring(0,d);e=e.replace(/，/g,",");var f=e.split(","),g=a.substring(d+3);this.invoker().withAbsUrl().require([g],function(a,d){for(var e=[],g=0;g<f.length;g++){var h=f[g],j=moduleScript.getModule(h);if(j&&"init"!=j.state&&!j.preDependModule){e.push("\tselfname="+h+",state="+j.state+",src="+j.src);continue}var k=d[0].module;if(j&&!j.preDependModule){j.toOtherModule(k);}else{moduleScript.setModule(h,k);}}if(e.length){c("already exists:"+e.join("\n"));}else{b(a);}}).error(function(a){c(a);});}});
  ...
  var L$s=U.global.xsloader;L$s.define("xsloader4j-server-bridge",[],function(){var b=new L$s.Base64;function a(a){return function(b,c){if(!a||!a.$createElement){var g=L$s.require.get("vue");a=new g;}var d=a.$createElement;if(!d){throw new Error("not found function '$createElement'")}for(var e=[],f=2;f<arguments.length;f++){e.push(arguments[f]);}return d(b,c,e)}}return {getDefine:function getDefine(a){return a.define},getRequire:function getRequire(a){return a.require},getInvoker:function getInvoker(a){return a.invoker()},renderJsx:function renderJsx(b){return a(b)},getVueCompiler:function getVueCompiler(a){return function rt(b){var c=L$s.require("vue"),d=b["default"]||b;if(d.template){var e=function(b,c,d,e,f){f=f||function(a,b){return b};var g="";while(true){var h=b.exec(d);if(!h){g+=d;break}else{var i=h[c].trim();g+=d.substring(0,h.index);if(L$s.startsWith(i,"url(")){g+=h[0];}else{g+=f(h,a.getUrl(i,e));}d=d.substring(h.index+h[0].length);}}return g};d.template=e(/(^|\s)(src|href)\s*=\s*('|")([^'"\(\)]+)('|")/,4,d.template,false,function(a,b){return a[2]+"="+a[3]+b+a[5]});d.template=e(/(^|\s)url\(([^\(\)]+)\)/,2,d.template,true);var f;try{if(false){console.log("compile vue template content,url=",a.getUrl());}f=c.compile(d.template);}catch(b){console.error("url=",a.getUrl());throw b}d.render=f.render;d.staticRenderFns=f.staticRenderFns;}delete d.template;}},getVtemplate:function getVtemplate(a){return function vtemplate(b){return function(c,d){a.invoker();a.require([b],function(a){c(a);}).error(function(a){d(a.err);});}}},getImporter:function getImporter(a){var b=this.getVtemplate(a);return function(a){return new Promise(b(a))}},getStyleBuilder:function getStyleBuilder(){return function(a){if(a){var b=L$s.randId(),c=0,d=document.createElement("style");d.setAttribute("id",b);d.setAttribute("type","text/css");d.innerHTML=a;return {init:function init(){if(0>=c){L$s.appendHeadDom(d);}c++;return {destroy:function destroy(){if(0>=--c){var a=document.getElementById(b);if(a){a.parentNode.removeChild(a);}}}}}}}}},decodeBase64:function decodeBase64(a){if(a){return b.decode(a)}}}});

  var G$7=U.global,L$t=G$7.xsloader,http=G$7._xshttp_request_,DATA_CONF="data-conf",DATA_CONFX="data-xsloader-conf",DATA_CONF2="data-conf2",DATA_CONF2X="data-xsloader-conf2",DATA_MAIN="data-main",DATA_MAINX="data-xsloader-main",DATA_CONF_TYPE="data-conf-type",serviceConfigUrl,dataConf=L$t.script().getAttribute(DATA_CONF)||L$t.script().getAttribute(DATA_CONFX),dataMain=L$t.script().getAttribute(DATA_MAIN)||L$t.script().getAttribute(DATA_MAINX),dataConfType=L$t.script().getAttribute(DATA_CONF_TYPE);if("json"!==dataConfType&&"js"!=dataConfType){dataConfType="auto";}var initFun=function(){function a(a){var b=a.main.getPath.call(a,dataMain),c=location.pathname,d=c.lastIndexOf("/"),e=c.substring(d+1);if(""===e){e="index";}if(L$t.endsWith(e,".html")){e=e.substring(0,e.length-5);}if(!b){b="./main/{name}.js";}b=b.replace("{name}",e);return b}function b(a){a=L$t.extendDeep({properties:{},main:{getPath:function getPath(){return dataMain||"./main/{name}.js"},name:"main",localConfigVar:"lconfig",globalConfigVar:"gconfig",before:function before(a){console.log("before:"+a);},after:function after(a){console.log("after:"+a);}},service:{hasGlobal:false,resUrls:[]},chooseLoader:function chooseLoader(){return "default"},loader:{default:{}}},a);return a}function c(d,f,g,h){http({url:f,method:"get",timeout:2e4,handleType:"text",ok:function ok(d){var i;if("js"==dataConfType){i=L$t.xsEval(d);}else if("json"==dataConfType){i=L$t.xsParseJson(d);}else{if(L$t.startsWith(f,location.protocol+"//"+location.host+"/")){i=L$t.xsEval(d);}else{i=L$t.xsParseJson(d);}}i=b(i);if(i.beforeDealProperties){i.beforeDealProperties();}i=L$t.dealProperties(i,i.properties);if(h&&i.service.hasGlobal){c("global servie",i.service.confUrl,function(b){var c=h;G$7[b.main&&b.main.localConfigVar||c.main.localConfigVar]=c;G$7[b.main&&b.main.globalConfigVar||c.main.globalConfigVar]=b;var d,f,g;g=b.chooseLoader.call(b,c);var h,i;if(null!=g){d=b.main.name;f=U.getPathWithRelative(location.href,a(b));i=b.loader[g];h=b;if(i){b.current=g;}}if(!i){g=c.chooseLoader.call(c,null);d=c.main.name;f=U.getPathWithRelative(location.href,a(c));i=c.loader[g];h=c;if(i){c.current=g;}}if(!i){console.error("unknown loader:"+g+"");return}e(d,f,i,h,c);});}else{g(i);}},fail:function fail(a){console.error("load "+d+" config err:url="+f+",errinfo="+a);}}).done();}function d(){c("local",serviceConfigUrl,function(b){G$7[b.main.localConfigVar]=b;var c=b.main.name,d=location.href,f=d.lastIndexOf("?");if(0<=f){d=d.substring(0,f);}var g=a(b);g=-1==g.indexOf("!")?U.getPathWithRelative(d,g):g;var h=b.chooseLoader.call(b,null),i=b.loader[h];if(!i){console.error("unknown local loader:"+h);return}b.current=h;e(d,c,g,i,b,b);},true);}function e(a,b,c,d,e,f){var g=[];e.service.resUrl&&g.push(e.service.resUrl);f!==e&&f.service.resUrl&&g.push(f.service.resUrl);e.service.resUrls&&Array.pushAll(g,e.service.resUrls);f!==e&&f.service.resUrls&&Array.pushAll(g,f.service.resUrls);L$t.resUrlBuilder=function(a){var b=[];U.each(g,function(c){b.push(L$t.appendArgs2Url(c,"m="+encodeURIComponent(a)));});return b};d.ignoreProperties=true;d.defineFunction=d.defineFunction||{};d.depsPaths=d.depsPaths||{};if(-1!=c.indexOf("!")){var h=L$t(d);b="_plugin_main_";var i=[];if(h.deps){if(h.deps["*"]){Array.pushAll(i,h.deps["*"]);}if(h.deps[b]){Array.pushAll(i,h.deps[b]);}}i.push(c);L$t.define(b,i,function(){}).then({absUrl:a});}else if(!L$t.hasDefine(b)){d.depsPaths[b]=c;L$t(d);}else{L$t(d);}d.defineFunction[b]=function(a,c,d){if(L$t.isFunction(e.main.before)){e.main.before.call(e,b);}var f=a.apply(c,d);if(L$t.isFunction(e.main.after)){e.main.after.call(e,b);}return f};L$t.require([b],function(){}).error(function(a,b){if(b){console.error("error occured:invoker.url=",b.getUrl());}console.error("invoke main err:");console.error(a);});}L$t.asyncCall(d,true);};if(dataConf){serviceConfigUrl=U.getPathWithRelative(location.href,dataConf);}else if(dataConf=L$t.script().getAttribute(DATA_CONF2)||L$t.script().getAttribute(DATA_CONF2X)){serviceConfigUrl=U.getPathWithRelative(L$t.scriptSrc(),dataConf);}else{initFun=null;}initFun&&initFun();
